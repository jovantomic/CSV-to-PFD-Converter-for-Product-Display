...
def generate_pdf(df, output_file, title):
    ...
    # Register and set the TrueType font
    ...
    container_spacing = 10  # Reduced spacing between containers
    ...
    for index, row in df.iterrows():
        ...
        if 'Ime' in df.columns and pd.notna(row['Ime']):
            c.drawString(text_x, text_y, f"Ime: {row['Ime']}")
            text_y -= line_height + text_spacing

        if 'Cena' in df.columns and pd.notna(row['Cena']):
            c.drawString(text_x, text_y, f"Cena: {row['Cena']} RSD")
            text_y -= line_height + text_spacing
